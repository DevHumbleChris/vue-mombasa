...
import { emailList } from '@/utils/formValidator'
...
const joinEmailList = async () => {
  // Form Validations.
  const { error } = await emailList(formDetails.value)
  if (error) {
    toast(error, {
      type: 'warning',
      theme: 'colored'
    ...
    //join email list logic goes here
...
<template>
  <section class="grid md:grid-cols-2 gap-4 my-6 sm:my-8 md:my-12 items-center mx-4">
    ...
    <div class="text-center">
      ...
      <h2 class="capitalize text-4xl my-4 text-bold font-comforta">
        We are comming soon!
      </h2>
      <div class="
          grid grid-cols-2
      ...
      <div class="my-6">
        <p class="text-lg">
          The
          <span class="text-secondary font-bold"> Vue Mombasa Community</span>
          website is opening soon! Please register to notify you when it's
          ready.
        </p>
        <a href="https://chat.whatsapp.com/CtNmD8cpogJLvzkh0AonO8" class="
            mx-auto
            flex
            space-x-3
            items-center
        ...
        <form @submit.prevent="joinEmailList" class="
            relative
            my-4
            mx-auto
            max-w-lg
          ...
          <div class="flex-1">
            ...
            <div>
              <input type="email" v-model="formDetails.email" id="email" autocomplete="off" class="
                  block
                  w-full
                  px-4
                  py-3
